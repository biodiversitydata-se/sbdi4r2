...
```{r filter_timerange, message=FALSE, fig.width=8, fig.height=6}
xf <- sbdi_call() |> 
  sbdi_identify(taxa) |>
  filter(year >= 2010, year <= 2020) |>
  atlas_occurrences()
...
```{r plot_sf, message=FALSE, fig.width=9, fig.height=9}
library(sf)
...
```{r plot_leaflet, message=FALSE, fig.width=9, fig.height=9}
library(leaflet)
...
```{r plot_counties, warning=FALSE, fig.width=6, fig.height=6}
...
```{r plot_countyna, warning=FALSE, fig.width=8, fig.height=6}
...
Vector spatial layers (eg. polygons) can be imported in a number of different ways and the package helps to pass those polygons into the search. 
So the first step is to load a vector spatial layer. Download a .zip file with different [delimitation for Sweden](https://www.scb.se/hitta-statistik/regional-statistik-och-kartor/regionala-indelningar/digitala-granser) and move it somewhere you like in your computer. We recommend you move it into your working directory (`getwd()`). Extract the .zip file named KommunSweref99.zip.
...
```{r plot_long, warning=FALSE, fig.width=8, fig.height=6}
library(ggplot2)
...
```{r plot_env, warning=FALSE, fig.width=8, fig.height=6}
...
```{r plot_points_map, fig.width=6, fig.height=3, message=FALSE, warning=FALSE}
